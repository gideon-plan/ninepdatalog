...
import lattice, fs_layout, fact_fs, query_fs
type
...
proc handle_read*(s: var DatalogServer, path: string): Result[string, BridgeError] =
  inc s.ops
  if path == result_path(s.root):
    return Result[string, BridgeError].good(read_result(s.query_state))
  let parsed = parse_fact_path(s.root, path)
  if parsed.valid: return read_fact(s.list_fn, parsed.predicate)
  Result[string, BridgeError].bad(BridgeError(msg: "invalid path: " & path))
proc handle_write*(s: var DatalogServer, path: string, data: string): Result[void, BridgeError] =
  inc s.ops
  ...
  Result[void, BridgeError].bad(BridgeError(msg: "invalid path: " & path))
